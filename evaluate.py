...
Output: Printed metrics + confusion matrix
...
Note: Takes ~20-60 seconds per entry. Full eval = ~3-8 hours.
      Use --sample for a quick test run.
"""
...
def run_evaluation(
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        ...
        eta_sec = (elapsed / (i + 1)) * (len(df) - i - 1) if i > 0 else 0
...
Examples:
  ...
  python evaluate.py --verbose    # Detailed output for each query
    ...
    # Run evaluation
    print("🚀 Starting evaluation (with RAG pipeline)...\n")
    results = run_evaluation(df, query_timeout=args.timeout, verbose=args.verbose)
    metrics = compute_metrics(results["true"], results["pred"])
    
    print_results(metrics, results["latencies"])
    ...
    print(f"  {'-'*70}")
    print(f"  Macro F1 Score (primary)      : {metrics['macro_f1']}")
    print(f"  Macro Precision               : {metrics['macro_precision']}")
    print(f"  Macro Recall                  : {metrics['macro_recall']}")
    print(f"  Accuracy                      : {metrics['accuracy']}")
    print(f"  True Positives / False Pos    : {metrics['tp']} / {metrics['fp']}")
    print(f"  True Negatives / False Neg    : {metrics['tn']} / {metrics['fn']}")
    print(f"  {'-'*70}")
    print(f"  Evaluation set size           : {len(df)} memes")
    print(f"  Average latency per query     : {sum(results['latencies'])/len(results['latencies']):.2f}s")
    print(f"  Total evaluation time         : {sum(results['latencies'])/60:.1f} minutes")
    print(f"{'='*70}\n")
